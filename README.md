...
このプロジェクトをローカル環境で動かすには、高速なパッケージマネージャである `uv` を使用します。
次の手順を実行してください。
...
# 仮想環境を作成し、ロックファイルから依存関係を同期する
uv sync
```

> [!NOTE]
> サプライチェーン攻撃対策として、パッケージリリースから7日以内の新しいバージョンを排除するように設定されています（`exclude-newer = "7 days"`）。

## 使用方法

アプリケーションを実行するには、`uv run` を使用します。これにより、自動的に安全にロックされた仮想環境が使用されます。
